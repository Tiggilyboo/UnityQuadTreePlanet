...
public class CubePlanet
{
    ...
    public CubePlanet(GameObject gameObject, int size, float scale)
    {
        ...
        for (int f = 0; f < faces.Length; f++)
            faces[f].gameObject.transform.parent = this.gameObject.transform;
    }
    ...
    /// Keeps track of LOD according to camera distance to each CubeFace
    ...
    public IEnumerator Update(int f)
    {
        ...
        {
            var dist = Vector3.Distance(camera.transform.position, activeTree[a].Mesh.bounds.ClosestPoint(camera.transform.position));
